...
// 二维正态分布
function GaussianDistribution(x, y, o1, o2, u1 = 0, u2 = 0, p = 0) {
  ...
  return (1 / (2 * Math.PI * o1 * o2 * Math.sqrt(1 - Math.pow(p, 2)))) * Math.exp(-1 / (2 * (1 - Math.pow(p, 2))) * (Math.pow(x - u1, 2) / Math.pow(o1, 2) - 2 * p * (x - u1) * (y - u2) / 2 * o1 * o2 + Math.pow(y - u2, 2) / Math.pow(o2, 2)));
}

export default filterMath;